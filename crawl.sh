...
ACCESS_TOKEN=$(retry 3 curl -fsS --max-time 5 https://sso.redhat.com/auth/realms/redhat-external/protocol/openid-connect/token -d grant_type=refresh_token -d client_id=rhsm-api -d "refresh_token=${REDHAT_API_OFFLINE_TOKEN}" | jq -r .access_token)
export ACCESS_TOKEN
