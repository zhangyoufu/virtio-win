...
rhsm_request() {
	...
	curl -fsS --max-time 5 "https://api.access.redhat.com/management/v1${URI}" -H "Authorization: Bearer ${ACCESS_TOKEN}" "$@" || {
		local EXIT_CODE=$?
...
process_package() {
	...
	local RELEASE_PUBLISH_TIME=$(awk -v k="${GIT_TAG}" '$1 == k {print $2}' "${TMPDIR}/known_github_release")
	...
	for FILEPATH in "${DIR}/usr/share/virtio-win/"*; do # nullglob not set, upload will fail when no file
		log "upload ${FILEPATH##*/}"
		retry 3 gh release upload --clobber "${GIT_TAG}" "${FILEPATH}"
	done
...
PARALLEL='--will-cite --jobs 5 --halt soon,fail=2'
...
CSET_LINES=$(export -f get_subscription_content_sets rhsm_list rhsm_request; parallel --retries 1 --group bash -c "'"'set -euo pipefail; get_subscription_content_sets "$1"'"'" _ <<<"${SUBSCRIPTION_JSON_LINES}")
...
PKG_LINES=$(export -f find_package_in_content_set rhsm_request; printf '%s\n' "${!CSETS[@]}" | parallel --retries 1 --group bash -c "'"'set -euo pipefail; find_package_in_content_set "$1"'"'" _)
if [ -z "${PKG_LINES}" ]; then
	log 'no package'
	exit 1
fi
...
gh release list --limit 99999 --json tagName,publishedAt --jq '.[] | "\(.tagName) \(.publishedAt|fromdateiso8601)"' >"${TMPDIR}/known_github_release"

log '# process packages'
(export -f process_package rhsm_request retry; printf '%s\n' "${!PKGS[@]}" | parallel --retries 1 --group bash -c "'"'set -euo pipefail; process_package "$1"'"'" _)
rm -f "${TMPDIR}/known_github_release"
